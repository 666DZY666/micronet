...
def test_quant_model_train():
    ...
    for data, target in testloader:
        ...
        quant_model_train_test_loss += criterion(
            quant_model_train_output, target).data.item()
        quant_model_train_pred = quant_model_train_output.data.max(1, keepdim=True)[1]
        quant_model_train_correct += quant_model_train_pred.eq(
            target.data.view_as(quant_model_train_pred)).cpu().sum()
    end_time = time.time()
    inference_time = end_time - start_time
    FPS = len(testloader.dataset) / inference_time

    quant_model_train_acc = 100. * \
        float(quant_model_train_correct) / len(testloader.dataset)
    ...
    print('\nquant_model_train: Test set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%), inference_time:{:.4f}ms, FPS:{:.4f}'.format(
        average_quant_model_train_test_loss, quant_model_train_correct,
        len(testloader.dataset), quant_model_train_acc, inference_time * 1000, FPS))
    return
