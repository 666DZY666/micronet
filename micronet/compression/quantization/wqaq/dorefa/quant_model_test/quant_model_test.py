...
def test_quant_model_inference():
    ...
    for data, target in testloader:
        ...
        quant_model_inference_test_loss += criterion(
            quant_model_inference_output, target).data.item()
        quant_model_inference_pred = quant_model_inference_output.data.max(1, keepdim=True)[1]
        quant_model_inference_correct += quant_model_inference_pred.eq(
            target.data.view_as(quant_model_inference_pred)).cpu().sum()
    end_time = time.time()
    inference_time = end_time - start_time
    FPS = len(testloader.dataset) / inference_time

    quant_model_inference_acc = 100. * \
        float(quant_model_inference_correct) / len(testloader.dataset)
    ...
    print('quant_model_inference: Test set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%), inference_time:{:.4f}ms, FPS:{:.4f}'.format(
        average_quant_model_inference_test_loss, quant_model_inference_correct,
        len(testloader.dataset), quant_model_inference_acc, inference_time * 1000, FPS))
    return
