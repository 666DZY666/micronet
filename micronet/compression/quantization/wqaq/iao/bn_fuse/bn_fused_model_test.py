...
def test_quant_bn_fused_model_train():
    ...
    for data, target in testloader:
        ...
        quant_bn_fused_model_train_test_loss += criterion(
            quant_bn_fused_model_train_output, target).data.item()
        quant_bn_fused_model_train_pred = quant_bn_fused_model_train_output.data.max(
            1, keepdim=True)[1]
        quant_bn_fused_model_train_correct += quant_bn_fused_model_train_pred.eq(
            target.data.view_as(quant_bn_fused_model_train_pred)).cpu().sum()
    end_time = time.time()
    inference_time = end_time - start_time
    FPS = len(testloader.dataset) / inference_time

    quant_bn_fused_model_train_acc = 100. * \
        float(quant_bn_fused_model_train_correct) / len(testloader.dataset)
    average_quant_bn_fused_model_train_test_loss = quant_bn_fused_model_train_test_loss / \
    ...
    print('\nquant_bn_fused_model_train: Test set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%), inference_time:{:.4f}ms, FPS:{:.4f}'.format(
        average_quant_bn_fused_model_train_test_loss, quant_bn_fused_model_train_correct,
        len(testloader.dataset), quant_bn_fused_model_train_acc, inference_time * 1000, FPS))
    return
...
def test_quant_bn_fused_model_inference():
    ...
    for data, target in testloader:
        ...
        quant_bn_fused_model_inference_test_loss += criterion(
            quant_bn_fused_model_inference_output, target).data.item()
        quant_bn_fused_model_inference_pred = quant_bn_fused_model_inference_output.data.max(
            1, keepdim=True)[1]
        quant_bn_fused_model_inference_correct += quant_bn_fused_model_inference_pred.eq(
            target.data.view_as(quant_bn_fused_model_inference_pred)).cpu().sum()
    end_time = time.time()
    inference_time = end_time - start_time
    FPS = len(testloader.dataset) / inference_time

    quant_bn_fused_model_inference_acc = 100. * \
        float(quant_bn_fused_model_inference_correct) / len(testloader.dataset)
    average_quant_bn_fused_model_inference_test_loss = quant_bn_fused_model_inference_test_loss / \
        (len(testloader.dataset) / args.eval_batch_size)

    print('quant_bn_fused_model_inference: Test set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%), inference_time:{:.4f}ms, FPS:{:.4f}'.format(
        average_quant_bn_fused_model_inference_test_loss, quant_bn_fused_model_inference_correct,
        len(testloader.dataset), quant_bn_fused_model_inference_acc, inference_time * 1000, FPS))
    return
...
if __name__ == '__main__':
    ...
    quant_bn_fused_model_train = torch.load('models_save/quant_bn_fused_model_train.pth')
    quant_bn_fused_model_train.eval()
    # quant_bn_fused_model_inference
    quant_bn_fused_model_inference = torch.load('models_save/quant_bn_fused_model_inference.pth')
