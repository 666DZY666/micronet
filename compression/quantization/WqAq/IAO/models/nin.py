...
class QuantConvBNReLU(nn.Module):
    def __init__(self, input_channels, output_channels,
                 kernel_size=-1, stride=-1, padding=-1, groups=1, abits=8, wbits=8, bn_fuse=0, q_type=1, q_level=0, first_layer=0):
...
class Net(nn.Module):
    def __init__(self, cfg = None, abits=8, wbits=8, bn_fuse=0, q_type=1, q_level=0):
        ...
        self.quant_model = nn.Sequential(
                ...
                QuantConvBNReLU(cfg[0], cfg[1], kernel_size=1, stride=1, padding=0, abits=abits, wbits=wbits, bn_fuse=bn_fuse, q_type=q_type, q_level=q_level),
                QuantConvBNReLU(cfg[1], cfg[2], kernel_size=1, stride=1, padding=0, abits=abits, wbits=wbits, bn_fuse=bn_fuse, q_type=q_type, q_level=q_level),
                QuantMaxPool2d(kernel_size=3, stride=2, padding=1, a_bits=abits, q_type=q_type),
                
                QuantConvBNReLU(cfg[2], cfg[3], kernel_size=5, stride=1, padding=2, abits=abits, wbits=wbits, bn_fuse=bn_fuse, q_type=q_type, q_level=q_level),
                QuantConvBNReLU(cfg[3], cfg[4], kernel_size=1, stride=1, padding=0, abits=abits, wbits=wbits, bn_fuse=bn_fuse, q_type=q_type, q_level=q_level),
                QuantConvBNReLU(cfg[4], cfg[5], kernel_size=1, stride=1, padding=0, abits=abits, wbits=wbits, bn_fuse=bn_fuse, q_type=q_type, q_level=q_level),
                QuantMaxPool2d(kernel_size=3, stride=2, padding=1, a_bits=abits, q_type=q_type),
                
                QuantConvBNReLU(cfg[5], cfg[6], kernel_size=3, stride=1, padding=1, abits=abits, wbits=wbits, bn_fuse=bn_fuse, q_type=q_type, q_level=q_level),
                QuantConvBNReLU(cfg[6], cfg[7], kernel_size=1, stride=1, padding=0, abits=abits, wbits=wbits, bn_fuse=bn_fuse, q_type=q_type, q_level=q_level),
