...
class Net(nn.Module):
    def __init__(self, cfg = None, abits=8, wbits=8):
        ...
        self.quant_model = nn.Sequential(
                ...
                QuantConvBNReLU(cfg[0], cfg[1], kernel_size=1, stride=1, padding=0, groups=2, channel_shuffle=0, abits=abits, wbits=wbits),
                QuantConvBNReLU(cfg[1], cfg[2], kernel_size=1, stride=1, padding=0, groups=2, channel_shuffle=1, shuffle_groups=2, abits=abits, wbits=wbits),
                nn.MaxPool2d(kernel_size=2, stride=2, padding=0),
                
                QuantConvBNReLU(cfg[2], cfg[3], kernel_size=3, stride=1, padding=1, groups=16, channel_shuffle=1, shuffle_groups=2, abits=abits, wbits=wbits),
                QuantConvBNReLU(cfg[3], cfg[4], kernel_size=1, stride=1, padding=0, groups=4, channel_shuffle=1, shuffle_groups=16, abits=abits, wbits=wbits),
                QuantConvBNReLU(cfg[4], cfg[5], kernel_size=1, stride=1, padding=0, groups=4, channel_shuffle=1, shuffle_groups=4, abits=abits, wbits=wbits),
                nn.MaxPool2d(kernel_size=2, stride=2, padding=0),
